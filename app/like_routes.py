...
async def report_nickname_abuse():
    try:
        ...
        @like_bp.route("/info", methods=["GET"])
async def get_player_info_all_tokens():
    try:
        uid = request.args.get("uid")
        region = request.args.get("region")
        print(f"[get_player_info_all_tokens] UID: {uid} | Region: {region}")

        if not uid or not uid.isdigit():
            print("[get_player_info_all_tokens] Invalid UID")
            return jsonify({
                "error": "Invalid UID",
        ...
        if not region or region not in _SERVERS:
            print("[get_player_info_all_tokens] Invalid region")
            return jsonify({
                "error": "Invalid region",
                "message": f"Supported regions: {', '.join(_SERVERS.keys())}",
                "status": 400,
        ...
        info_url = f"{_SERVERS[region]}/GetPlayerPersonalShow"
        encoded_uid = encode_uid(uid)

        async with aiohttp.ClientSession() as session:

            async def fetch_info(token):
                try:
                    headers = {
                        "Authorization": f"Bearer {token}",
                        "Content-Type": "application/json"
                    }
                    payload = {"uid": encoded_uid}
                    async with session.post(info_url, headers=headers, json=payload) as resp:
                        if resp.status == 200:
                            return await resp.json()
                        else:
                            print(f"[fetch_info] Failed with token {token[:6]}..., status: {resp.status}")
                            return None
                except Exception as e:
                    print(f"[fetch_info] Exception with token {token[:6]}...: {e}")
                    return None

            tasks = [fetch_info(token) for token in tokens]
        ...
        sample_info = None
        if success_count > 0:
            # Supondo estrutura parecida com seu decode_info, aqui direto do json:
            info_json = successful_responses[0]
            player_name = info_json.get("AccountInfo", {}).get("PlayerNickname", "N/A")
            player_likes = info_json.get("AccountInfo", {}).get("Likes", "N/A")
            sample_info = {"nickname": player_name, "likes": player_likes}
        else:
            sample_info = {"nickname": "N/A", "likes": "N/A"}
        ...
        return jsonify({
            ...
            "nickname": sample_info["nickname"],
            "likes": sample_info["likes"],
            "status": "ok" if success_count > 0
